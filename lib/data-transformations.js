...
export const contentfulThumbnailToImageUrl = (thumbnailGraphql) =>
  thumbnailGraphql?.imageBynder
    ? `${thumbnailGraphql.imageBynder[0]?.src}?${imageoptimization.qualityParameter}=${imageoptimization.qualityValue}`
    : thumbnailGraphql?.imageContentful?.file?.url
    ? `${thumbnailGraphql?.imageContentful?.file?.url}?${imageoptimization.qualityParameter}=${imageoptimization.qualityValue}`
    : undefined
...
export const accessInfoToMembershipData = (userAccesses) => {
  ...
  return {
    ...
    expireDate: dateFormat(
      new Date(membershipAccess?.expire_date * 1000),
      'longDate'
    ),
